...
<div class="data_list">
		...
		<form name="myForm" class="form-search" method="post" action="record?action=search" style="padding-bottom: 0px">
				<button class="btn btn-success" type="button" style="margin-right: 50px;" onclick="javascript:window.location='record?action=preSave'">add</button>
			...
			<table class="table table-hover table-striped table-bordered">
				...
				<c:forEach  varStatus="i" var="record" items="${recordList }">
					<tr>
						...
						<td>${record.detail }</td>
						<td>
							<button class="btn btn-mini btn-success" type="button" onclick="javascript:window.location='record?action=preSave&recordId=${record.recordId }'">modify</button>
