...
<title>Вход в систему управления общежитиями</title>
...
<script type="text/javascript">
	function checkForm() {
		...
		if (userName == null || userName == "") {
			document.getElementById("error").innerHTML = "Имя пользователя не может быть пустым";
			return false;
		}
		if (password == null || password == "") {
			document.getElementById("error").innerHTML = "Пароль не может быть пустым";
			return false;
		}
		if (userType == null || userType == "") {
			document.getElementById("error").innerHTML = "Пожалуйста, выберите тип пользователя";
			return false;
		}
		return true;
	}
...
<div class="container">
      <form name="myForm" class="form-signin" action="login" method="post" onsubmit="return checkForm()">
        <h2 class="form-signin-heading"><font color="gray">Система управления общежитиями</font></h2>
        ...
        <label class="radio inline">
      	  	<input id="admin" type="radio" name="userType" value="admin"  checked/> admin
		</label>
		<label class="radio inline">
			<input id="dormManager" type="radio" name="userType" value="dormManager" ${userType==2?'checked':''} /> Заведующий общежитием
		</label>
		<label class="radio inline">
			<input id="student" type="radio" name="userType" value="student"  ${userType==3?'checked':''}/> студенты
		</label>
        <label class="checkbox">
          <input id="remember" name="remember" type="checkbox" value="remember-me" ${remember==1?'checked':''}>remeber me &nbsp;&nbsp;&nbsp;&nbsp; <font id="error" color="red">${error }</font>
        </label>
        <button class="btn btn-large btn-primary" type="submit">войти</button>
        &nbsp;&nbsp;&nbsp;&nbsp;
        <button class="btn btn-large btn-primary" type="button" >перепрофилирование</button>
